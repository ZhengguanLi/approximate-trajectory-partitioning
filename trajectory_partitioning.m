...
num_vec_tra = T_all(3:end, 2); % the number of vectors
T_mat = T_all(3:end, 3:end); % data part
T = cell(1, size(T_mat, 1));

for i = 1:size(T_mat, 1)
    T{i} = reshape(T_mat(i, 1:num_vec_tra(i) * dim_tra), dim_tra, []);
    T{i} = T{i}'; % doing this because reshape is based on column
end
...
%% original path
function MDL_no = MDL_nonpar(start_idx, current_idx)
    global point_list;
    sum_ = 0;
